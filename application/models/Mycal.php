...
class Mycal extends CI_Model {


	function get_calendar_data($year, $month) {

		$query = $this->db->select('date, data')->from('main')
	...
	function generate ($year, $month) {
		...
		$conf['template'] = '
						...
						{heading_previous_cell}<th><a href="{previous_url}" class="darker">&lt;&lt;</a></th>{/heading_previous_cell}
						{heading_title_cell}<th colspan="{colspan}" class="darker fs-4">{heading}</th>{/heading_title_cell}
						{heading_next_cell}<th><a href="{next_url}" class="darker">&gt;&gt;</a></th>{/heading_next_cell}
...
/* Location: ./application/models/mycal.php */
